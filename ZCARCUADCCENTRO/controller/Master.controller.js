...
sap.ui.define([
	...
	return BaseController.extend("app.inetum.ZCARCUADCENTRO.controller.Master", {
		...
		onUpdateFinished: function(oEvent) {
			...
			/*if (!oModelNav || ( oModelNav && !oModelNavData.retailstoreid &&  )) {*/
			if (
		...
		onUpdateFinished_old: function(oEvent) {
			...
			if (vTotal > 0 && !this.gNavtoObject) {
				this._oList.getItems()[0].setSelected(true);
				var sObjectId = this._oList.getItems()[0].getBindingContext().getPath().slice(1);
					...
					oItems.forEach(function(item) {
						...
						item.getAttributes().forEach(function(att) {
							...
							if (oPath === "Zzcount1" && oValeTpv) {
								att.setText(oValeTpv.Canttpvcuad);
							}
						})
					});
		...
		onSearch: function(oEvent) {
			...
			if (sQuery) {
				...
				this._oListFilterState.aSearch.push(new Filter("Auditbusinessdaydate", FilterOperator.EQ, new Date(this.gDatePicker.getTime() -
					this.gDatePicker
					.getTimezoneOffset() *
					180000)));
			} else {
				/*this._oListFilterState.aSearch = [];*/
				this._oListFilterState.aSearch = [(new Filter("Auditbusinessdaydate", FilterOperator.EQ, new Date(this.gDatePicker.getTime() -
					this.gDatePicker
					.getTimezoneOffset() *
					180000)))];
			}
		...
		onOpenDatePicker: function() {
			...
			if (!this._oDatePickerDialog) {
				this._oDatePickerDialog = sap.ui.xmlfragment(oView.getId(), "app.inetum.ZCARCUADCENTRO.Fragments.MasterDate", this);
